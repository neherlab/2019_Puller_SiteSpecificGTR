...
if __name__ == '__main__':
    ...
    model_name = hiv_out + dset_name+'_aligned.%1.2f_inferred_model%s.npz'%(args.pc, '_aa' if args.aa else '')
    ...
    for r in range(20):
        ...
        for t in np.linspace(0,1,31):
            ...
            tmp_branch_length.append((t, gtr.optimal_t_compressed((s1_prof, s2_prof_q), multiplicity = np.ones_like(gtr.mu), profiles=True),
                    gtr_flat.optimal_t_compressed((s1_prof, s2_prof_q), multiplicity = np.ones_like(gtr.mu), profiles=True), 1-np.mean(s1[-1]==s2[-1])))
    ...
    axs[0].errorbar(branch_length_mean[:,0], branch_length_mean[:,2], branch_length_std[:,2], label='flat model')
    ...
    for fmt in ['.png', '.pdf']:
        plt.savefig(f'figures/{dset_name}_branch_length_error_pc_{args.pc:1.3f}{fmt}')
