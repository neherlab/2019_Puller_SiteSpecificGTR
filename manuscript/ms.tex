...
\begin{figure*}[htb]
	...
	The estimates are vastly more accurate than naive estimates from the state frequencies in alignment columns. Different curves are for $n\in [100,300,1000,3000]$ sequences per tree.
...
Underestimation is particularly severe for the average root-to-tip distance which is dominated by long branches deep in the phylogeny which are particularly prone to underestimation.
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=0.48\textwidth]{{../figures/model_deviation}.pdf}
...
The principle contributor to this deviation are inaccuracies in the rate estimates and combining the true rates with inferred preferences largely eliminates the error in branch length estimation (see Fig.~\ref{fig:tree_length}, lines labeled ``(true rates)''.
