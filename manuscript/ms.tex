...
\begin{figure*}[tb]
	...
	\caption{Errors stemming from tree and ancestral reconstruction: At large root-to-tip distances, ancestral reconstruction becomes more and more uncertain and estimation of equilibrium state frequencies fails.
	These errors are gradually eliminated by first summing over ancestral uncertainty, iterative redoing ancestral reconstruction using the inferred model, and re-optimizating branch length using the updated models (or using the true tree).
	The right panel shows analysis of simulated with four states (nucleotides), while the right panels shows the corresponding graph of 20 states (amino acids). As expected, saturation and non-linear effects set in later with larger alphabet.}
...
Similar improvements are achieved by optimizing tree branch length along with the model.



\subsection*{Ignoring site specific frequencies dramatically underestimates branch lengths}
As previously observed by various authors \citep{halpern_evolutionary_1998,hilton_modeling_2018}, sites with heavily skewed preferences for specific states result in branch length underestimation if uniform equilibrium frequencies are assumed.
This is a straightforward consequence of the fact that the probability of observing the same state at random is $\sum_i{\eqp_i^b}^2$ is increasing sharply with the skew.
...
While branch length are accurately estimated for when using the true model, they are systematically underestimated with increasing pseudo-counts and or without site specificity.
...
The inferred root-to-tip distance is essentially independent from the true value for distance greater than 1.
...
\begin{figure}[tb]
	\centering
	\includegraphics[width=0.48\textwidth]{{../figures/model_deviation_2019-06-01_simulatedData_L1000_ratealpha3.0_alpha1.0_nuc_n100_m0.35}.pdf}
...
In the previous section, we have shown that even small model deviations, for example those introduced by strong regularization, already result in substantial mis-estimation.
...
Deviations in rate and preferences are approximatly additive.
...
Initially, errors accumulate linearly with increasing.
...
Simple population genetic models predict that the fixation probability of state $i$ if starting from state $j$ depends on the fitness different $s_{ij}$ and the effective population size $N$ as
\begin{equation}
	f_{ij} = \frac{1-e^{-2s_{ij}}}{1-e^{-2Ns_{ij}}} \approx
	\begin{cases}
		2s_{ij} & s_{ij}>0 \\
		2s_{ij} e^{2Ns_{ij}} & s_{ij} < 0
	\end{cases}
\end{equation}
On longer time scales, the fixation rates $f_{ij}$ should correspond to transitions rates of the site specific GTR model $\muQ_{ij}$.
...
Nevertheless, the logarithm of the ratio $\log f_{ij}/f_{jj} \approx 2Ns_{ij}$ is an interpretable quantity related to the average fitness difference between states on time scales longer than the population genetic scale $N\sim T_c$.
We generalize this notion to multiple states and define a fitness score of state $i$ at position $a$ as the ratio of rates into and out of state $a$
\begin{equation}
...
Fig.~\ref{fig:HIV_fitness} shows a linear relation ship between the logarithmic fitness effects measured with-in host and from the phylogeny that explains about half the variance.
The linear relationship holds both for nucleotide states and amino acid states.
...
\begin{figure*}[tb]
	...
	\caption{{\bf Intra-host vs cross-sectional mutation selection balance are consistent. } }
...
\section*{Discussion}
Sequence evolution models attempt to capture some biological realism while being computationally tractable and easy to parameterize and a common compromise is to model rate heterogeneity as random effects, while assuming identical site preferences across the sequence.
Different positions in a DNA sequence or protein, however, also differ in the preferences for different states.
...
Here, we introduce and implement a method to estimate site specific GTR models using an iterative EM-stype approach inspired by the original paper by \citet{bruno1996modeling}.
...
The computational complexity of the inference is dominated by ancestral reconstruction and branch length optimization, which is quadratic in the alphabet size and linear in sample size and sequence length.

We further explore how branch length estimation depends on model choice and inference accuracy.
As has been reported before, branch length are underestimated with variation in rate or preferences are not fully accounted for \citep{halpern1998evolutionary,hilton_modeling_2018}.
Even moderate model deviation can result in substantial underestimates.
...
The deep branches, however, tend to be much longer and rate estimation and variation of site specific preferences will result in saturation effects not accoutned for by the model \citep{hilton_modeling_2018}.
This effective variation in rate is related to the effect of transient deletrious mutations that inflate the rate of short time scales.
...
\section*{Iterative optimization algorithm}
