...
Modeling this complexity, however, requires an extensive number or parameters that can not be infered from small data sets and are costly to infer from large data sets.
Most commonly used evolutionary models therefore reduce this complexity by partitioning sites into discrete classes with different rates and/or preferences where appropriate complexity is determined by model selection procedures.
...
We find that an iterative approximate maximum likelihood scheme uses information in the data efficiently and accurately estimates site-specific preferences from large data sets with moderately diverged sequences and known topology.
...
Here, we ask study under what circumstances models that allow different state frequencies at every site in the alignment can be estimated and what implications such variation has for phylogenetic inference.
...
Furthermore, the estimation of this extensive number of parameters requires large data sets in which most positions mutate multiple times on the tree -- otherwise the model will overfit the data and return inaccurate estimates.
...
As expected, the average squared error $\chi^2$ of the $\eqp_i^a$ decreases with the substitution rate and the size of the data set.
